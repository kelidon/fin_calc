...
class ButtonWidget extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CalculationBloc, CalculationState>(
        buildWhen: (prev, curr) => prev.operations[blocOperationIndex]!= curr.operations[blocOperationIndex],
      builder: (_, state) {
        bool isActive = state.operations[blocOperationIndex] == operation;
        return Container(
          decoration: BoxDecoration(
            color: isActive?Colors.black.withOpacity(0.5):Colors.white.withOpacity(0.5),
